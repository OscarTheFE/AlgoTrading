# -*- coding: utf-8 -*-
"""
Created on Sun Mar 27 08:58:57 2022
...
class TAQRegression(object):
    ...
    def preprocessing(self, start_date, ticker_list=None, splitting_by_liquidity=False):
        ...
        def clean_matrix(X, h, s, V, max_null_in_day=40, max_null_in_ticker=3):
    ...
    def bootstrapping_residual(self, start_date, m):
        ...
        return eta_list, beta_list, error_list
    ...
    def residual_property(self, error_list):
        
        #check statistics of the Error 
        
        jarque_bera_test = stats.jarque_bera(error_list)
        Error_mean = np.mean(error_list)
        
        Error_std = np.sqrt(np.var(error_list))
        Error_skew = np.mean(stats.skew(error_list))
        Error_kurtosis = np.mean(stats.kurtosis(error_list))
        
        print('Jarque_bera_stats')
        print(jarque_bera_test[0])
        print('jarque_beta_stats_p-value')
        print(jarque_bera_test[1])
        print('Error Mean')
        print(Error_mean)
        print('Error Variance')
        print(Error_std)
        print('Error Skewness') 
        print(Error_skew)
        print('Error Kurtosis')
        print(Error_kurtosis)
        
        # plot the Histogram of the residuals:
        _ = plt.figure(figsize = (15,10))
        pd.DataFrame(error_list).mean(axis = 1).plot(kind = 'hist', title = 'redidual histogram plot')
        
        return 
    
    def optimized_param(self, eta, beta, start_date):
        X_, h_, s_, V_ = self.preprocessing(start_date)
        ...
        modelPredictions = curve_fit_func((X_, s_, V_), eta, beta)
        
        error = pd.DataFrame({'error_value' : modelPredictions - h_ })
        
        exog_variable = pd.DataFrame({'X_': X_, 's_':s_, 'V_':V_})
        
        #return 
        return error, exog_variable 
        
        
        
    def hetero_check(self, error, exog_variable):
        # Conduct white's test 
        
        white_test = het_white(error, exog_variable)
...
if __name__ == "__main__":
    ...
    eta_1, beta_1, error_1 = TAQRegression_obj.bootstrapping_residual("20070907", 40)
    eta_2, beta_2, error_2 = TAQRegression_obj.bootstrapping_paired("20070907", 40)
    ...
    TAQRegression_obj.residual_property(error_1)
    
    error_opt, exog_variable_opt = TAQRegression_obj.optimized_param(np.average(eta_1), np.average(beta_1),"20070907" )
    
    print('t statistics for residual methods')
    TAQRegression_obj.get_t_statistic(eta_1, beta_1)
    print('t statistics for paired methods')
    TAQRegression_obj.get_t_statistic(eta_2, beta_2)

    print('liquidity comparison analysis')
    TAQRegression_obj.compare_params_by_liquidity("20070907")
    
    print('heterokasicity check')
    TAQRegression_obj.hetero_check(error_opt, exog_variable_opt)
